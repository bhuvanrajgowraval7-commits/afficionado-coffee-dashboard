...
# Product types now come ONLY from the filtered data
...
# The key changes when Store/Category changes.

# This prevents Streamlit from keeping an invalid old
...
product_summary["revenue"]
.div(
product_summary["units_sold"].replace(0, pd.NA)
)
.fillna(0)
)
...
# Create a unique product selector.

#

# Using product_detail alone can be unsafe if two products

# have the same name/detail.
...
```
# Create a filter-specific widget key.
# This guarantees that changing filters creates a
# fresh valid product selection.
